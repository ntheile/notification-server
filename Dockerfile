...
RUN --mount=type=cache,target=/usr/local/cargo,from=rust:latest,source=/usr/local/cargo \
    ...
    cargo build --target $(cat rust_target) --release && mv ./target/$(cat rust_target)/release/mutiny-notifications ./mutiny-notifications
...
COPY --from=builder /usr/src/app/mutiny-notifications /app/mutiny-notifications
